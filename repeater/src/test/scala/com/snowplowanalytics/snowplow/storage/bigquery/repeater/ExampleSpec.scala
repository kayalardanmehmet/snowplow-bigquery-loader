...
class ExampleSpec extends Specification { def is = s2"""
  test test $e1
  """
  import ExampleSpec._

  def e1 = {
    val res = PayloadParser.parse(failedInsertRow.asObject.get)
      .map(_.asJson).leftMap((_: BadRow).asJson)
    println(res)
    ok
  }
}
